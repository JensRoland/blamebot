...
func extractEdits(data map[string]interface{}, projectDir string) []editInfo {
	...
	case "Edit":
		...
		lines, hunk := editChangedLines(oldStr, newStr, toolResponse)
		return []editInfo{{
			File:        filePath,
			Lines:       lines,
			Hunk:        hunk,
	...
	case "MultiEdit":
		...
		for i, editRaw := range subEdits {
			...
			if i < len(patches) {
				...
				lines, hunk = editChangedLines(oldStr, newStr, subResp)
			}
...
// Also returns hunk metadata for shift computation.
func editChangedLines(oldStr, newStr string, toolResponse map[string]interface{}) (lineset.LineSet, *record.HunkInfo) {
	patches := getArray(toolResponse, "structuredPatch")
	if len(patches) == 0 {
	...
	return lineset.ChangedLines(oldStr, newStr, newStart), hunk
}
